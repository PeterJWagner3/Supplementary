...
tally_boundary_crossers_from_binned_synoptic_data <- function(synoptic_data)	{
ntaxa <- nrow(synoptic_data)
mxbin <- max(synoptic_data$la)

boundary_crossers <- vector(length=mxbin);
for (nt in 1:ntaxa)	
	if (synoptic_data$la[nt]>synoptic_data$fa[nt])	{
		bin_span <- synoptic_data$fa[nt]:(synoptic_data$la[nt]-1);
		boundary_crossers[bin_span] <- boundary_crossers[bin_span]+1;
		}
return(boundary_crossers);
}
...
# written 2022-04-14
# modified 2024-01-12
accersi_range_through_and_three_timer_stats_from_pbdb_data <- function(taxa,paleodb_finds,paleodb_sites,finest_chronostrat,taxon_rank="species")	{
ntaxa <- length(taxa);
n_ints <- nrow(finest_chronostrat);
if (taxon_rank=="species")	taxon_rank <- "accepted_name";
# redo bin_lb & bin_ub even if they alread are there just in case we've reduced the time scale;
if (is.null(paleodb_sites$ma_lb))	{
	age <- paleodb_sites$max_ma;
	paleodb_sites$bin_lb <- match(pbapply::pbsapply(age,rebin_collection_with_time_scale,onset_or_end="onset",fine_time_scale=finest_chronostrat),finest_chronostrat$interval);
	age <- paleodb_sites$min_ma;
	paleodb_sites$bin_ub <- match(pbapply::pbsapply(age,rebin_collection_with_time_scale,onset_or_end="end",fine_time_scale=finest_chronostrat),finest_chronostrat$interval);
	} else	{
	age <- paleodb_sites$ma_lb;
	paleodb_sites$bin_lb <- match(pbapply::pbsapply(age,rebin_collection_with_time_scale,onset_or_end="onset",fine_time_scale=finest_chronostrat),finest_chronostrat$interval);
	age <- paleodb_sites$ma_ub;
	paleodb_sites$bin_ub <- match(pbapply::pbsapply(age,rebin_collection_with_time_scale,onset_or_end="end",fine_time_scale=finest_chronostrat),finest_chronostrat$interval);
	}
...
observed_shared <- array(0,dim=c(n_ints,n_ints));
...
#	taxon_finds <- pbdb_finds[pbdb_finds$genus==taxa[g],];
	taxon_finds <- paleodb_finds[paleodb_finds[,taxon_rank]==taxa[tx],];
	sites <- tx_sites <- match(unique(taxon_finds$collection_no),paleodb_sites$collection_no);
	taxon_sites <- paleodb_sites[tx_sites,]
	sites_def <- subset(paleodb_sites[tx_sites,],paleodb_sites$bin_lb[tx_sites]==paleodb_sites$bin_ub[tx_sites]);
...
# N: lineages at outset
# n: number of lineages going extinct.
#mu <- log(1+n/N)/chrons;
mu <- -log(N-n/N)/chrons;
return (mu)
}

per_capita_birth_or_death_rates <- function(n,N,chrons=1)	{
# N: lineages at outset/end that might go extinct or originate
# n: number of lineages at end/onset that didn't go extinct or originate
# chrons: duration of interval
#mu <- log(1+n/N)/chrons;
drate <- -log(n/N)/chrons;
return (drate)
}
