...
accersi_rev_bayes_friendly_names <- function(otus)	{
notu <- length(otus);
otu_is_subgenus <- otus[sapply(otus,is.subgenus)];
i <- 0;
while (i < length(otu_is_subgenus))	{
	i <- i+1;
	j <- match(otu_is_subgenus[i],otus);
	otus[j] <- elevate_subgenus_to_genus(otu_is_subgenus[i])
	}

species_in_subgenus <- otus[sapply(otus,is.species.in.subgenus)]
i <- 0;
while (i < length(species_in_subgenus))	{
	i <- i+1;
	j <- match(species_in_subgenus[i],otus)
	otus[j] <- paste(divido_subgenus_names_from_genus_names(divido_genus_names_from_species_names(species_in_subgenus[i]))[2],divido_species_epithets(species_in_subgenus[i]));
	}
...
divido_character_matrix_by_ordering <- function(nexus_file_name,data_directory="",script_directory="",output_file_lead="output/",script_file_lead="script/",analysis_name="",no_runs=4,write_rev_bayes_source=TRUE,polymorphs=TRUE, UNKNOWN=-11, INAP=-22)	{
...
otus <- initial_data$OTUs;
chmatrix <- initial_data$Matrix;
rownames(chmatrix) <- otus;
n_chars <- length(n_states);
state_orders <- initial_data$State_Type;
state_orders[state_orders=="ordered" & n_states==2] <- "unordered";
unique_orderings <- unique(state_orders);
rownames(chmatrix) <- accersi_rev_bayes_friendly_names(otus);

ch_matrix_lists <- list();
if (length(unique_orderings)>1)	{
	for (uo in 1:length(unique_orderings))	{
		new_file_name <- strsplit(nexus_file_name,"/")[[1]][length(strsplit(nexus_file_name,"/")[[1]])]
		new_file_name <- gsub(".nex","",new_file_name);
		subname <- unique_orderings[uo];
		subname <- strsplit(subname,"")[[1]]
		subname[1] <- toupper(subname[1]);
		subname <- paste(subname,collapse="");
		new_file_name <- paste(new_file_name,"_",subname,"_for_RevBayes.nex",sep="");
		these_char <- (1:nchars)[state_orders %in% unique_orderings[uo]];
		or_matrix <- array(chmatrix[,these_char],dim=c(notu,length(these_char)));
		rownames(or_matrix) <- otus;
		ch_matrix_lists <- rlist::list.append(ch_matrix_lists,or_matrix);
		names(ch_matrix_lists)[i] <- new_file_name;
		scribio_nexus_file_from_chmatrix(ch_matrix=or_matrix,new_file_name);
		}
	} else	{
	new_file_name <- strsplit(nexus_file_name,"/")[[1]][length(strsplit(nexus_file_name,"/")[[1]])]
	new_file_name <- gsub(".nex","",new_file_name);
	new_file_name <- paste(new_file_name,"_for_RevBayes.nex",sep="");
	ch_matrix_lists <- rlist::list.append(ch_matrix_lists,chmatrix);
	names(ch_matrix_lists)[1] <- new_file_name;
	scribio_nexus_file_from_chmatrix(chmatrix,new_file_name);
	}
return(ch_matrix_lists);
}
...
#	revbayes_source <- c(revbayes_source,paste("monitors[++mni] = mnFile(tau, filename=\"Documents/RevStudio_Projects/output/",tolower(analysis_name),".trees\",printgen=1000,separator=TAB,tau);\t\t#Play with these numbers",sep=""));
	...
	revbayes_source <- c(revbayes_source,"");
	revbayes_source <- c(revbayes_source,paste("source(\"",script_file_lead,"standard_routines/AbracaMCMC_with_Partitioned_Characters.Rev\");",sep=""));
	revbayes_source <- c(revbayes_source,"");
...
divido_character_matrix_by_state_numbers_and_other_partitions <- function(nexus_file_name,analysis_name="",first_appearances=NULL,data_directory="",write_data_directory="",ind_char_rate_partitions="",int_char_trend_partitions="",taxa_subset="",data_file_lead="data/",polymorphs=T, fix_unique_polymorphs=F,UNKNOWN=-11, INAP=-22,coding_threshold=1)	{
...
if (data_directory!="")	nexus_file_name <- paste(data_directory,nexus_file_name,sep="");
initial_data <- accersi_data_from_nexus_file(nexus_file_name,polymorphs,UNKNOWN,INAP,ind_char_rate_partitions,int_char_trend_partitions);
...
#if (write_rev_bayes_source)	{
nstates <- sort(unique(n_states));
ind_char_rate_partitions <- initial_data$Rate_Partitions;
int_char_trend_partitions <- initial_data$Trend_Partitions;

# separate out all unique combinations that we might use to partition the characters
if (length(unique(ind_char_rate_partitions))>1 && length(unique(int_char_trend_partitions))>1)	{
	partition_combos <- data.frame(n_states=as.numeric(n_states),type=as.character(state_types),rate_partitions=as.character(ind_char_rate_partitions),trend_partitions=as.character(int_char_trend_partitions),stringsAsFactors=hell_no);
	...
	} else if (length(unique(ind_char_rate_partitions))>1)	{
	partition_combos <- data.frame(n_states=as.numeric(n_states),type=as.character(state_types),rate_partitions=as.character(ind_char_rate_partitions),stringsAsFactors=hell_no);
	partition_combos <- unique(partition_combos);
	partition_combos <- partition_combos[order(partition_combos$n_states,partition_combos$type),];
	} else if (length(unique(int_char_trend_partitions))>1)	{
	partition_combos <- data.frame(n_states=as.numeric(n_states),type=as.character(state_types),trend_partitions=as.character(int_char_trend_partitions),stringsAsFactors=hell_no);
	partition_combos <- unique(partition_combos);
	partition_combos <- partition_combos[order(partition_combos$n_states,partition_combos$type),];
	} else	{
...
while (pc < nrow(partition_combos))	{
	...
	if (length(unique(ind_char_rate_partitions))>1)	{
		# find characters in this rate partitioning
		right_partition <- (1:n_chars)[ind_char_rate_partitions %in% partition_combos$rate_partitions[pc]];
		relv_characters <- relv_characters[relv_characters %in% right_partition];
		}
	if (length(unique(int_char_trend_partitions))>1)	{
		# find characters in this trend partitioning
		right_partition <- (1:n_chars)[int_char_trend_partitions %in% partition_combos$trend_partitions[pc]];
		relv_characters <- relv_characters[relv_characters %in% right_partition];
		}
	...
	if (fix_unique_polymorphs)	{
		...
		while (ppf < length(poss_polymorph_fixs))	{
			...
			while (potu < length(poly_otus))	{
				potu <- potu+1;
				other_taxon_states <- c();
				for (nn in 1:notu)	{
					if (nn!=poly_otus[potu])	{
						if (chmatrix_red[nn,nch]>0)	{
							other_taxon_states <- sort(unique(c(other_taxon_states,chmatrix_red[nn,nch])));
							} else if (!chmatrix_red[nn,nch] %in% c(INAP,UNKNOWN))	{
							other_taxon_states <- sort(unique(c(other_taxon_states,unravel_polymorph(chmatrix_red[nn,nch]))));
							}
						}
					}
				polystates <- unravel_polymorph(chmatrix_red[poly_otus[potu],nch]);
				if (sum(!polystates %in% other_taxon_states)==1)	{
					chmatrix_red[poly_otus[potu],nch] <- polystates[!polystates %in% other_taxon_states];
					}
...
divido_chosen_character_matrix_file_by_state_numbers_and_other_partitions <- function(analysis_name="",first_appearances=NULL,write_data_directory="",rate_partitions="",trend_partitions="",taxa_subset="",data_file_lead="data/",polymorphs=T,fix_unique_polymorphs=F,UNKNOWN=-11,INAP=-22,coding_threshold=1)	{
# rate_partitions: name of acharacter group that should have separate rates (e.g., CHARPARTITION Functional_Partitions  =  Nonfeeding :  1- 45 58- 60, Feeding :  46- 57; in a Nexus file)
# trend_partitions: name of character group that for which one group ("Driven") has biased change (e.g., Driven_Trend  =  Unbiased :  1- 32 34- 40 43- 50 52- 60, Driven :  33 41- 42 51; in a Nexus file)
# taxa_subset: vector listing the subset of species to include in the final matrices;
# write_data_directory: the directory to which the partitioned matrices should go
# data_file_lead: the directory addendum to paste in front of data files so that RevBayes can find them
if (analysis_name=="")	analysis_name <- "Wombat_Rock";
...
#print("Reached Line 423");
#if (write_rev_bayes_source)	{
nstates <- sort(unique(n_states));
ind_char_rate_partitions <- initial_data$Rate_Partitions;
ind_char_trend_partitions <- initial_data$Trend_Partitions;
#print("Reached Line 428");
# separate out all unique combinations that we might use to partition the characters
if (length(unique(ind_char_rate_partitions))>1 && length(unique(ind_char_trend_partitions))>1)	{
	partition_combos <- data.frame(n_states=as.numeric(n_states),type=as.character(state_types),rate_partitions=as.character(ind_char_rate_partitions),trend_partitions=as.character(ind_char_trend_partitions),stringsAsFactors=hell_no);
	partition_combos <- unique(partition_combos);
	partition_combos <- partition_combos[order(partition_combos$n_states,partition_combos$type),];
	} else if (length(unique(ind_char_rate_partitions))>1)	{
	partition_combos <- data.frame(n_states=as.numeric(n_states),type=as.character(state_types),rate_partitions=as.character(ind_char_rate_partitions),stringsAsFactors=hell_no);
	partition_combos <- unique(partition_combos);
	partition_combos <- partition_combos[order(partition_combos$n_states,partition_combos$type),];
	} else if (length(unique(ind_char_trend_partitions))>1)	{
	partition_combos <- data.frame(n_states=as.numeric(n_states),type=as.character(state_types),trend_partitions=as.character(ind_char_trend_partitions),stringsAsFactors=hell_no);
	partition_combos <- unique(partition_combos);
	partition_combos <- partition_combos[order(partition_combos$n_states,partition_combos$type),];
	} else	{
	partition_combos <- data.frame(n_states=as.numeric(n_states),type=as.character(state_types),stringsAsFactors=hell_no);
...
#print("Reached Line 447");
if (taxa_subset!="" && length(taxa_subset)<notu)	{
	chmatrix <- chmatrix[match(taxa_subset,rownames(chmatrix)),];
	notu <- nrow(chmatrix);
	}
#print(paste("Reached Line 452",fix_unique_polymorphs));
pc <- 0;
partition_size <- character_rate_partitions <- character_trend_partitions <- matrix_file_names <- coding_bias <- c();
while (pc < nrow(partition_combos))	{
	# check coding_bias here!!!!
	...
	if (length(unique(ind_char_trend_partitions))>1)	{
		# find characters in this trend partitioning
		right_partition <- (1:n_chars)[ind_char_trend_partitions %in% partition_combos$trend_partitions[pc]];
		relv_characters <- relv_characters[relv_characters %in% right_partition];
		}
	# look for invariant & autapomorphic characters
	chmatrix_red <- chmatrix[,relv_characters];
	if (!is.matrix(chmatrix_red))	chmatrix_red <- as.matrix(chmatrix_red);
	## RevBayes ignores polymorphs; so, if the only cases of derived states are in polymorphic taxa, recode them to derived states;
	observed_states_sans_poly <- count_states(chmatrix_red,UNKNOWN,INAP,include_polymorphs = F);
#	print("Reached Line 481");
	if (fix_unique_polymorphs)	{
		poss_polymorph_fixs <- (1:ncol(chmatrix_red))[observed_states_sans_poly<partition_combos$n_states[pc]];
		ppf <- 0;
		while (ppf < length(poss_polymorph_fixs))	{
			...
			poly_otus <- (1:notu)[!chmatrix_red[,nch] %in% c(INAP,UNKNOWN,0:partition_combos$n_states[pc])];
			orig_state <- obs_states <- sort(unique(chmatrix_red[chmatrix_red[,nch]>=0,nch]));
			potu <- 0; #print("Reached Line 490");
			while (potu < length(poly_otus))	{
				potu <- potu+1;
				other_taxon_states <- c(); #print(paste("Reached Line 493",fix_unique_polymorphs));
				for (nn in 1:notu)	{
					if (nn!=poly_otus[potu])	{
						if (chmatrix_red[nn,nch]>0)	{
							other_taxon_states <- sort(unique(c(other_taxon_states,chmatrix_red[nn,nch]))); #print(paste("Reached Line 497",nn,nch,poss_polymorph_fixs,fix_unique_polymorphs));
							} else if (!chmatrix_red[nn,nch] %in% c(INAP,UNKNOWN))	{
							other_taxon_states <- sort(unique(c(other_taxon_states,unravel_polymorph(chmatrix_red[nn,nch])))); #print("Reached Line 499");
							}
						}
					}
				polystates <- unravel_polymorph(chmatrix_red[poly_otus[potu],nch]);
...
#	print("Reached Line 516");
	...
	if (write_data_directory!="")
		new_file_name <- paste(write_data_directory,new_file_name,sep="");
	state_symbols <- accersi_state_symbols(n_states=partition_combos$n_states[pc]);
#	print("Reached Line 539");
	scribere_rev_bayes_nexus_file_from_character_matrix(ch_matrix=chmatrix_red,state_symbols = state_symbols,new_file_name=new_file_name,UNKNOWN,INAP);
	matrix_file_names <- c(matrix_file_names,paste(data_file_lead,orig_file_name,sep=""));
	}
#print("Reached Line 543");
state_numbers <- partition_combos$n_states;
state_ordering <- partition_combos$type;
if (!is.null(partition_combos$rate_partitions))	{
	character_rate_partitions <- partition_combos$rate_partitions;
...
#print("Reached Line 557");
output <- list(initial_data,matrix_file_names,partition_size,state_numbers,state_ordering,character_rate_partitions,character_trend_partitions,coding_bias);
names(output) <- c("initial_data","matrix_file_names","partition_size","state_numbers","state_ordering","rate_partitions","trend_partitions","coding_bias");
return(output);
}

divido_nexus_file_by_state_numbers_and_other_partitions <- function(nexus_file_name,analysis_name="",first_appearances=NULL,write_data_directory="",rate_partitions="",trend_partitions="",taxa_subset="",data_file_lead="data/",polymorphs=T, UNKNOWN=-11, INAP=-22,coding_threshold=1,coding_bias="all")	{
# rate_partitions: name of acharacter group that should have separate rates (e.g., CHARPARTITION Functional_Partitions  =  Nonfeeding :  1- 45 58- 60, Feeding :  46- 57; in a Nexus file)
...
revbayes_source <- c(revbayes_source,paste("source(\"",paste(script_file_lead,"standard_routines/Milgram_Default_Settings.Rev",sep=""),"\");",sep=""));
revbayes_source <- c(revbayes_source,"");
revbayes_source <- c(revbayes_source,"###############################################################################");
...
#	revbayes_source <- c(revbayes_source,paste("source(\"",script_file_lead,fbd_parameterization_script),"\");",sep="");
	...
	revbayes_source <- c(revbayes_source,paste("source(\"",paste(script_file_lead,"FBD_scripts\\",fbd_parameterization_script,sep=""),"\");",sep=""));
	} else	{
	revbayes_source <- c(revbayes_source,"############################################################################");
	revbayes_source <- c(revbayes_source,"# Set up tree-search moves");
	revbayes_source <- c(revbayes_source,"############################################################################");
...
#	revbayes_source <- c(revbayes_source,paste("intervals = readDataDelimitedFile(file=\"data/",fossil_interval_file,"\",header=true);",sep=""));
	xxx <- strsplit(fossil_interval_file,"/")[[1]];
	fbd_file_name <- xxx[length(xxx)];
	last_cell <- match("RevBayes_Projects",xxx);
	if (is.na(last_cell))	last_cell <- match("R_Projects",xxx);
	xxx[last_cell+1] <- data_file_lead;
	directory_lead <- paste(xxx[1:(last_cell+1)],collapse="/")
	revbayes_source <- c(revbayes_source,paste("taxa <- readTaxonData(file=\"",data_file_lead,fbd_file_name,"\");",sep=""));
	revbayes_source <- c(revbayes_source,paste("n_taxa <- taxa.size();"));
...
revbayes_source <- c(revbayes_source,paste("source(\"scripts/standard_routines/AbracaMCMC_with_Partitioned_Characters.Rev\");",sep=""));
revbayes_source <- c(revbayes_source,"# .......");
...
#scribere_RevBayes_scripts_from_nexus_file_and_PaleoDB_download <- function(analysis_name,nexus_file_name,taxon_subset_file="",rate_partitions="",trend_partitions="",taxon_level,lump_subgenera=F,species_only=T,bogarted="",rock_unit_databases="",chronostratigraphic_databases="",paleodb_fixes="",control_taxon="",zone_taxa="",onset="Cambrian",end="Holocene",end_FBD="",exclude_uncertain_taxa=T,basic_environments=c("terr","marine","unknown"),sampling_unit="collections",time_scale_stratigraphic_scale="International",temporal_precision=0.1,read_data_directory="",write_data_directory="",write_scripts_directory="",local_directory="",set_wdir="",UNKNOWN=-11,INAP=-22)	{
scribere_RevBayes_scripts_from_nexus_file_and_PaleoDB_RData <- function(analysis_name,taxon_subset_file=F,rate_partitions="",trend_partitions="",taxon_level="species",lump_subgenera=F,species_only=T,bogarted="",pbdb_data_list,control_taxon="",zone_taxa="",onset="Cambrian",end="Holocene",end_FBD="",exclude_uncertain_taxa=T,sampling_unit="collections",time_scale_stratigraphic_scale="International",temporal_precision=0.1,write_data_directory="",write_scripts_directory="",local_directory="",set_wdir="",polymorphs=T,UNKNOWN=-11,INAP=-22)	{
#### PART 0: Commence ####
print("This program will read a Nexus file and then create scripts that RevBayes can use to conduct phylogenetic analyses,");
print("    using data downloaded from the Paleobiology Database (https://www.paleobiodb.org/) for stratigraphic data and then");
print("    for refining/cleaning/updating those data with updated time scales and biozonation information.");
print("");
print("NOTE: This uses an RData file of 'refined' PBDB data with more exact dates. Even after this sort of refining,");
print("\tthe Paleobiology Database should always be considered a STARTING point for these data. This program will also");
print("\toutput summaries of the data that you should check. We encourage you to contribute improvements to these data");
print("\t(to collections, occurrences and/or taxonomy) to the Paleobiology Database.  Improvements to the stratigraphic");
print("\tdatabase used to refine PaleoDB data should be sent to pjwagner@gmail.com");
print("");
...
if (taxon_subset_file)	{
	...
	print("Choose the nexus file that you wish to analyze: ");
                                                                   ...
                                                                   function(nexus_file_name,analysis_name="",first_appearances=NULL,data_directory="",write_data_directory="",ind_char_rate_partitions="",int_char_trend_partitions="",taxa_subset="",data_file_lead="data/",polymorphs=T, fix_unique_polymorphs=F,UNKNOWN=-11, INAP=-22,coding_threshold=1)
basic_data <- divido_character_matrix_by_state_numbers_and_other_partitions(analysis_name=analysis_name,write_data_directory=write_data_directory,ind_character_rate_partitions=rate_partitions,int_character_trend_partitions=trend_partitions,taxa_subset=taxa_subset,data_file_lead="data/",polymorphs=polymorphs,UNKNOWN=UNKNOWN,INAP=INAP);
initial_data <- basic_data$initial_data;
matrix_file_names <- basic_data$matrix_file_names;
state_numbers <- basic_data$state_numbers;
state_ordering <- basic_data$state_ordering;
...
if (!is.null(fa_info) && ncol(fa_info)>2)	{
...
basic_data <- divido_chosen_character_matrix_file_by_state_numbers_and_other_partitions(analysis_name,first_appearances=fa_info,write_data_directory,rate_partitions=rate_partitions,trend_partitions=trend_partitions,taxa_subset=taxa_subset,data_file_lead=data_file_lead,polymorphs=T,UNKNOWN=UNKNOWN,INAP=INAP,coding_threshold=coding_threshold);
#basic_data <- divido_nexus_file_by_state_numbers_and_other_partitions(analysis_name,first_appearances=fa_info,write_data_directory,rate_partitions,trend_partitions,taxa_subset,data_file_lead=data_file_lead,polymorphs=T,UNKNOWN,INAP,coding_threshold = coding_threshold);
initial_data <- basic_data$initial_data;
matrix_file_names <- basic_data$matrix_file_names;
...
output <- list(revbayes_stone_babble,revbayes_mcmc_babble);
names(output) <- c("stepping_stone_script","mcmc_script");
return(output);
...
if (taxon_level %in% c("species","subspecies"))	{
	clade_members <- unique(sapply(ingroup_taxa,divido_genus_names_from_species_names));
	} else	{
	clade_members <- ingroup_taxa;
	}
...
#tn <- 0;
if (notu >= 100)	{
	update_numbers <- round((1:19)*notu/20,0);
	} else	{
	update_numbers <- round((1:9)*notu/10,0);
	}
#while (tn < notu)	{
for (tn in 1:notu)	{	
#	tn <- tn+1;
#	if (tn%%5==0)	print(tn);
	if (tn %in% update_numbers)
		print(paste(100*match(tn,update_numbers)/(1+length(update_numbers)),"% done",sep=""))
	taxon_aliases <- pbdb_taxonomy$taxon_name[pbdb_taxonomy$accepted_name %in% pbdb_taxonomy$accepted_name[match(taxon_names[tn],pbdb_taxonomy$taxon_name)]]
	if (length(taxon_aliases)==0)	{
		if (!is.subgenus(divido_genus_names_from_species_names(taxon_names[tn])))	{
			try_this <- paste(divido_genus_names_from_species_names(taxon_names[tn])," (",divido_genus_names_from_species_names(taxon_names[tn]),") ",divido_species_epithets(taxon_names[tn]),sep="");
			taxon_aliases <- pbdb_taxonomy$taxon_name[pbdb_taxonomy$accepted_name %in% pbdb_taxonomy$accepted_name[match(try_this,pbdb_taxonomy$taxon_name)]];
			}
		}
	taxon_alias_nos <- pbdb_taxonomy$taxon_no[pbdb_taxonomy$accepted_name %in% pbdb_taxonomy$accepted_name[match(taxon_names[tn],pbdb_taxonomy$taxon_name)]]
	taxon_finds <- paleodb_finds[paleodb_finds$accepted_name %in% taxon_aliases,]
	if (nrow(taxon_finds)==0)	{
		# we whiffed! Let's try looking for an updated combination
		tnr <- unique(which(pbdb_taxonomy==taxon_names[tn],arr.ind = T)[,1]);
		taxon_finds <- paleodb_finds[paleodb_finds$accepted_no %in% taxon_alias_nos,];
		}
	if (nrow(taxon_finds)==0)	{
		# we whiffed! Let's try looking for an updated combination
		tnr <- unique(which(pbdb_taxonomy==taxon_names[tn],arr.ind = T)[,1]);
...
#	if (nrow(taxon_finds)==0 && )	{
#		divido_genus_names_from_species_names()
#		}
...
rev_bayes_data_1 <- data.frame(taxon=as.character(basic_data$OTUs),
...
rev_bayes_data_2 <- data.frame(taxon=as.character(basic_data$OTUs),
...
if (local_directory!="")	{
	...
	output_filenames[1] <- paste(analysis_name,"_Fossil_Info.tsv",sep="");
	output_filenames[2] <- paste(analysis_name,"_Fossil_Info_Full.tsv",sep="");
	output_filenames[3] <- paste(analysis_name,"_Full_Chronostratigraphic_Data.csv",sep="");
	}
