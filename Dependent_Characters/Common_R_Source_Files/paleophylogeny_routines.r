...
accersi_minimum_divergences <- function(vtree,strat_ranges,apos,min_prec=0.1)	{
# vtree: vector giving node from which lineage evolved
# strat_ranges: matrix giving first & last appearances
# apos: apomorphies along each branch
# dstate: numbers of derived states
# min_prec: minimum
notu <- min(vtree[vtree>0])-1;
gaps <- accersi_gaps_from_vector_tree(vtree,strat_ranges[1:notu,],apos);
dates <- date_clades_from_vector_tree(vtree,FA=strat_ranges$FA[1:notu]);
...
ohtu <- length(vtree);
node_poss_anc <- accersi_poss_ancestors_for_nodes(vtree,FA=strat_ranges$FA,apos)[(notu+1):ohtu]
for (nn in Nnode:1)	{
...
accersi_simple_clock_divergences <- function(vtree,strat_ranges,apos,dstates,min_prec=0.1)	{
# vtree: vector giving node from which lineage evolved
# strat_ranges: matrix giving first & last appearances
# apos: apomorphies along each branch
# dstate: numbers of derived states
...
states <- count_states(chmatrix=cdata);
char_state_combs <- c();
for (ch in 1:length(states))	{
...
#	sort(unique(cvector));
	if (sum(cvector>=0)>0)	{
		...
		full_matrix[,c] <- char_evolution$States;
...
# 2020-09-01: updated to accommodate states appearing only in polymorphics
# method for reconstructing ancestral conditions.
# cvector <- cvector[1:otus]
...
#	print(ancestral_states[1,1]);
	...
	for (nch in 1:nchars)	{
		...
		other_states <- (1:states[nch])[!(1:states[nch]) %in% orig_state];
		...
		for (os in 1:length(other_states))	{
			...
			if (nrow(this_state)>1)	{
				...
				for (ro in 1:nrow(this_state))	{
					...
					} # finish looking at derivations of this state states
				} # finish examining other states
	#		if (nrow(this_state)>1)	{
	#			print(nch);
	#			}
			}
...
# likelihood routines
seed_Mk_rate <- function(vtree,chmatrix,steps,durations,add_myr,UNKNOWN=-11,INAP=-22)	{
notu <- min(vtree[vtree>0]) - 1;
ghosts <- durations$FA[1:notu]-durations$LA[vtree[1:notu]];
...
accersi_tree_log_likelihood_simple <- function(m_alpha,divergence_times,vtree,notu,marginals,states)	{
...
alpha_sts <- vector(length=max(states))
for (st in 2:max(states))	alpha_sts[st] <- m_alpha/(1+st-2)
ttu <- max(vtree)
nNodes <- ttu-notu
mxstates <- max(states)
tnchars <- length(states)
...
accersi_tree_log_likelihood_simple_dud <- function(m_alpha,divergence_times,vtree,notu,marginals,states)	{
# divergence_times: vector giving 
# vtree: vector giving htu number of node from which each htu & otu evolved
#print(m_alpha);
alpha_sts <- vector(length=max(states));
...
accersi_tree_marginals_simple <- function(m_alpha,divergence_times,vtree,notu,marginals,states)	{
# divergence_times: vector giving 
# vtree: vector giving htu number of node from which each htu & otu evolved
#
alpha_sts <- vector(length=max(states))
...
accersi_tree_bds_probability_simple <- function(m_alpha,divergence_times,vtree,notu,marginals,states,BDS)	{
# divergence_times: vector giving 
# vtree: vector giving htu number of node from which each htu & otu evolved
#
alpha_sts <- vector(length=max(states))
...
accersi_tree_bds_probability_given_basal_divergence_and_rate <- function(divergence_times,onset,m_alpha,vtree,notu,marginals,states,BDS)	{
# divergence_times: vector giving 
# vtree: vector giving htu number of node from which each htu & otu evolved
#
alpha_sts <- vector(length=max(states))
...
accersi_tree_log_likelihood_simple_check_nodes <- function(node_ages,m_alpha,vtree,notu,fas,marginals,states)	{
...
accersi_tree_likelihood_divergences_scaled_to_base <- function(base,m_alpha,divergence_times,vtree,notu,marginals,states)	{
# divergence_times: vector giving 
# vtree: vector giving htu number of node from which each htu & otu evolved
# node_ages <- -470+runif(nNodes)*30
...
# do another version of this in which we optimize the rate!
accersi_tree_likelihood_divergences_scaled_to_base_w_two_rates <- function(base,m_alpha_1,m_alpha_2,branch_rates,divergence_times,vtree,notu,marginals,states)	{
# divergence_times: vector giving 
# vtree: vector giving htu number of node from which each htu & otu evolved
# node_ages <- -470+runif(nNodes)*30
first_sample <- min(divergence_times[1:notu])
...
for (st in 2:max(states))	alpha_sts[st] <- m_alpha_1/(1+st-2)
mxstates <- max(states)
tnchars <- length(states)
#marginals <- array(0,dim=c(ttu,tnchars,mxstates))
logmarginals <- array(0,dim=c(nNodes,tnchars,mxstates))
...
# m_alpha <- 0.01307749
# m_alpha <- 0.01082389
# marginals <- allocate_node_marginals(chmatrix_2,nttu,states)
optimo_rate_divergences_scaled_to_base_old <- function(base,m_alpha,divergence_times,vtree,notu,marginals,states)	{
# divergence_times: vector giving 
# vtree: vector giving htu number of node from which each htu & otu evolved
# node_ages <- -470+runif(nNodes)*30
#print(base);
...
optimo_rate_and_tree_divergences_posterior_scaled_to_base_two_rates <- function(base,m_alpha_1,m_alpha_2,divergence_times,branch_rates,vtree,notu,marginals,states,BDS,rate_quants=1)	{
# divergence_times: vector giving 
# vtree: vector giving htu number of node from which each htu & otu evolved
# node_ages <- -470+runif(nNodes)*30
#print(base);
...
optimo_rate_and_tree_divergences_likelihood_scaled_to_base <- function(base,m_alpha,divergence_times,vtree,notu,marginals,states,rate_quants=1)	{
# divergence_times: vector giving 
# vtree: vector giving htu number of node from which each htu & otu evolved
# node_ages <- -470+runif(nNodes)*30
#print(base);
...
optimo_rate_divergences_scaled_to_base_bds_redone <- function(base,m_alpha,divergence_times,vtree,notu,marginals,states,BDS,rate_quants=1)	{
# divergence_times: vector giving 
# vtree: vector giving htu number of node from which each htu & otu evolved
# node_ages <- -470+runif(nNodes)*30
...
node_divergences <- divergences[(notu+2):length(vtree)];
...
#sapply(m_alpha,accersi_tree_log_likelihood_simple,)								divergence_times,                 vtree,      notu,     marginals,          states,       BDS)
ddd <- optim(m_alpha,fn=accersi_tree_bds_probability_simple,method="L-BFGS-B",divergence_times=divergence_times,vtree=vtree,notu=notu,marginals=marginals,states=states,BDS=BDS,lower=m_alpha/10,upper=1.0,control=cl)
ml_alpha <- ddd$par
mlgln <- ddd$value
...
optimo_rate_divergences_scaled_to_base_bds <- function(base,m_alpha,divergence_times,vtree,notu,marginals,states,BDS)	{
# divergence_times: vector giving 
# vtree: vector giving htu number of node from which each htu & otu evolved
# node_ages <- -470+runif(nNodes)*30
#original_divergence_times <- divergence_times;
...
rescale_early_divergences <- function(base,divergence_times,vtree,notu)	{
# divergence_times: vector giving 
# vtree: vector giving htu number of node from which each htu & otu evolved
# node_ages <- -470+runif(nNodes)*30
first_sample <- min(divergence_times[1:notu])
...
Mk_punctuated_varying_rates <- function(k,alphas_p,lambdas,m,bd)	{
...
for (b in 1:bins)	{
	...
	rate <- rate+(alphas_p[b]*(mb+labdas[b]*bd[b]))
	}
...
accersi_starter_divergences_min <- function(vtree,simple_clock,strat_ranges,apos,prec=0.1)	{
...
mtree_to_date <- transform_vector_tree_to_matrix_tree(vector_tree=vtree);
notu <- min(vtree[vtree>0])-1;
...
date_taxa_on_tree_for_plotting <- function(vtree,FAs,tie_break=1/3)	{
ttus <- max(vtree)	# total htus+otus
dates <- vector(length=ttus)
notu <- length(FAs)	# number of otus
if (length(dim(as.array(vtree)))==1)	{
	...
	grandparents <- sort(unique(vtree[(base+1):ttus]),decreasing=TRUE)
	for (g in 1:length(grandparents))	{
		htu <- grandparents[g]
		dn <- ((base+1):ttus)[vtree[(base+1):ttus]==htu]
		if (length(dn)>0)	{
...
accersi_node_durations <- function(vtree,FAs)	{
divergences <- date_clades_from_vector_tree(vtree,FAs);
notu <- min(vtree[vtree>0])-1;
htu <- -sort(unique(-vtree[vtree>0]));

node_durations <- c();
for (ht in 1:length(htu))	{
	if (vtree[htu[ht]]>0)	{
		node_durations <- rbind(c(divergences[vtree[htu[ht]]],divergences[htu[ht]]),node_durations);
		} else	{
...
# good for optimizing a single rate or a mean rate given a distribution (summarized in rate_quants)
# modified 2020-07-11
likelihood_of_early_and_late_alphas_given_divergences <- function(m_alphas,divergence_times,branch_rates,vtree,marginals,states,rate_quants=1)	{
# m_alpha: typical rate for character change
# divergence_times: vector giving 
# vtree: vector giving htu number of node from which each htu & otu evolved
# states: number fo state per character
...
log_like <- apply(alphas,MARGIN=1,FUN=calculate_early_and_late_rate_likelihood_given_tree_divergences_and_characters,vtree=vtree,divergence_times=divergence_times,branch_rates=branch_rates,marginals=marginals,states=states);
#log_like <- c();
#for (rq in 1:nrow(alphas))
#	log_like <- c(log_like,calculate_early_and_late_rate_likelihood_given_tree_divergences_and_characters(alphas=alphas[rq,],vtree=vtree,divergence_times=divergence_times,branch_rates=branch_rates,marginals=marginals,states=states));
#	log_like <- sapply(alphas,calculate_early_and_late_rate_likelihood_given_tree_divergences_and_characters,vtree=vtree,divergence_times=divergence_times,branch_rates=branch_rates,marginals=marginals,states=states);
...
# created 2020-07-11
likelihood_of_early_and_late_alphas_given_divergences_shift <- function(m_alphas,divergence_times,boaty_mcboatface="",vtree,marginals,states,shift,rate_quants=1)	{
# m_alpha: typical rate for character change
# divergence_times: vector giving 
# vtree: vector giving htu number of node from which each htu & otu evolved
# states: number fo state per character
...
alpha_2 <- m_alpha_2*rate_quants;
alphas <- cbind(m_alpha_1*rate_quants,m_alpha_2*rate_quants);
#log_like <- apply(alphas,MARGIN=1,FUN=calculate_early_and_late_rate_likelihood_given_tree_divergences_and_characters,vtree=vtree,divergence_times=divergence_times,branch_rates=boaty_mcboatface,marginals=marginals,states=states);
log_like <- apply(alphas,MARGIN=1,FUN=calculate_early_and_late_rate_likelihood_given_tree_divergences_and_characters_shift,vtree=vtree,divergence_times=divergence_times,shift=shift,marginals=marginals,states=states);
#log_like <- c();
#for (rq in 1:nrow(alphas))
#	log_like <- c(log_like,calculate_early_and_late_rate_likelihood_given_tree_divergences_and_characters(alphas=alphas[rq,],vtree=vtree,divergence_times=divergence_times,branch_rates=branch_rates,marginals=marginals,states=states));
#	log_like <- sapply(alphas,calculate_early_and_late_rate_likelihood_given_tree_divergences_and_characters,vtree=vtree,divergence_times=divergence_times,branch_rates=branch_rates,marginals=marginals,states=states);
...
calculate_early_and_late_rate_likelihood_given_tree_divergences_and_characters <- function(alphas,vtree,divergence_times,branch_rates,marginals,states,rate_quants=1)	{
notu <- match(-1,vtree)-1;
tree_base <- notu+1;
ttu <- length(vtree);
nNodes <- ttu-notu;
...
# created 2020-07-11
calculate_early_and_late_rate_likelihood_given_tree_divergences_and_characters_shift <- function(alphas,vtree,divergence_times,shift,marginals,states,rate_quants=1)	{
notu <- match(-1,vtree)-1;
tree_base <- notu+1;
ttu <- length(vtree);
nNodes <- ttu-notu;
...
#	b_rate <- branch_rates[htu];
	...
	for (ch in 1:tnchars)
		marginals[htu,ch,1:states[ch]] <- 1;	
...
center_budding_phylogeny <- function(vtree,durations,sampled_ancestors)	{
# vtree: tree as a vector with ancestral node HTU given for each taxon (-1 for basal node)
# durations: origins and ends
# function to get relative positions of lineages on a phylogeny;
venntree <- transform_vector_tree_to_venn_tree(vtree);
mtree <- transform_vector_tree_to_matrix_tree(vtree);
node_richness <- accersi_node_richness_from_vector_tree(vector_tree = vtree);
nNode <- nrow(mtree);
notu <- length(vtree)-nNode;
node_ages <- c();
...
ladderize_vector_tree <- function(vtree)	{
lvtree <- lvtree_old <- vtree;
venn_tree <- transform_vector_tree_to_venn_tree(vtree);
notu <- max(venn_tree);
ttu <- length(vtree);
matrix_tree <- transform_vector_tree_to_matrix_tree(vtree);
node_rich <- tally_node_richness_from_vector_tree(vtree)
node_rich <- c(rep(0,notu),node_rich)
for (nd in 1:nrow(matrix_tree))	{
	matrix_tree_old <- matrix_tree;
	daughters <- matrix_tree[nd,matrix_tree[nd,]>0];
	tomy <- 1:length(daughters);
	old_daughter_clades <- daughters[daughters>notu];
	new_daughters <- daughters[order(node_rich[daughters])];
	new_daughter_clades <- new_daughters[new_daughters>notu];
	if (length(new_daughter_clades)>1)	{
		jj <- 1:(length(new_daughter_clades)-1);
		kk <- 2:length(new_daughter_clades);
		if (sum((new_daughter_clades[jj]-new_daughter_clades[kk])>0)>0)	{
			for (dd in 1:length(new_daughter_clades))
				lvtree[(1:ttu)[lvtree_old==old_daughter_clades[dd]]] <- new_daughter_clades[dd];
			}
		}
	mat_tree <- transform_vector_tree_to_matrix_tree(lvtree);
	}
lvenn_tree <- transform_vector_tree_to_venn_tree(vector_tree=lvtree);

lvtree <- transform_matrix_tree_to_vector_tree(matrix_tree);
venn_tree <- venn_tree[order(-node_rich),];
lvtree <- transform_venn_tree_to_vector_tree(venn_tree);
return(lvtree);
}
...
mtree <- transform_vector_tree_to_matrix_tree(vector_tree);
tree_dates <- date_taxa_on_tree_simple(vtree=vector_tree,FAs=fad);
notu <- length(fad)
clades <- nrow(mtree);
...
tree_dates <- date_taxa_on_tree_simple(vtree=vector_tree,FAs=strat_ranges$FAD);
